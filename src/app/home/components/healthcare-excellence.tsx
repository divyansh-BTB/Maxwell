    ...
    >
      <div className="mx-auto grid max-w-[1280px] grid-cols-1 items-center gap-[clamp(40px,8vw,100px)] lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <div className="relative w-full max-w-[500px] mx-auto lg:max-w-none">
          ...
          <div className="absolute -bottom-6 -right-6 top-6 left-6 rounded-[2rem] border-[1.5px] border-[#1F9AAA]/40 transition-transform duration-700 hover:translate-x-2 hover:translate-y-2 lg:-bottom-8 lg:-right-8 lg:top-8 lg:left-8" />

          {/* Main Image Container */}
          <div className="group relative aspect-[4/5] w-full overflow-hidden rounded-[2rem] shadow-2xl shadow-[#0E4A4F]/10">
            <Image
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="mb-10 lg:mb-14">
            ...
            <h2 className="mb-6 text-[clamp(2rem,3.5vw,3rem)] font-bold leading-[1.15] text-[#0E4A4F]">
              Specialized healthcare hospitality solutions.
            </h2>
            <p className="max-w-[90%] text-[1.05rem] leading-[1.8] text-[#5C7078]">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-6">
            {stats.map((stat, index) => (
              <div
                className="group relative flex min-h-[140px] cursor-default flex-col justify-end overflow-hidden rounded-2xl border border-[#E7F1F5] bg-white p-6 transition-all duration-500 ease-out hover:-translate-y-1 hover:border-[#0E4A4F] hover:bg-[#0E4A4F] hover:shadow-xl hover:shadow-[#0E4A4F]/20"
                key={stat}
              >
                {/* Accent Corner that expands on hover */}
                <div className="absolute -right-6 -top-6 h-16 w-16 rounded-full bg-white transition-all duration-500 ease-out group-hover:scale-[15] group-hover:bg-[#1F9AAA] z-0" />

                {/* Content */}
                <div className="relative z-10">
                  {/* Small gold indicator line */}
                  <div className="mb-4 h-1 w-8 rounded-full bg-[#E7F1F5] transition-all duration-500 group-hover:w-12 group-hover:bg-[#1F9AAA]" />

                  <strong className="block pr-4 text-[1.15rem] leading-[1.3] text-[#0E4A4F] transition-colors duration-500 group-hover:text-white">
                    {stat}
                  </strong>
                </div>

                {/* Background Watermark Number - Now visible by default and glows on hover */}
                <span className="absolute bottom-2 right-4 z-0 text-6xl font-black text-[#0E4A4F]/10 transition-colors duration-500 group-hover:text-white/15">
                  0{index + 1}
                </span>
              </div>
            ))}
