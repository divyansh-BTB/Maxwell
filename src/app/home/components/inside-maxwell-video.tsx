...
export function InsideMaxwellVideo() {
  ...
  return (
    <>
      <button
        className="group mt-7 inline-flex cursor-pointer items-center gap-3.5 text-left"
        onClick={() => setIsOpen(true)}
        ref={triggerRef}
        type="button"
      >
        <span className="relative flex h-11 w-11 shrink-0 items-center justify-center">
          <span className="absolute inset-0 animate-ping rounded-full bg-[#0D2B44]/20 [animation-duration:2.6s] motion-reduce:animate-none" />
      ...
      {isOpen && isMounted
        ? createPortal(
            // Rendered on document.body so no transformed ancestor (e.g. the
            // hero entrance animation) can become the containing block for
            // this fixed overlay.
