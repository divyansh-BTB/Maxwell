...
export function AboutHero() {
  return (
    <section className="relative bg-[#FAFAF8] overflow-hidden" id="about-hero">
      ...
      <div className="relative flex h-[60vh] min-h-[500px] w-full flex-col justify-end overflow-hidden px-[clamp(20px,5vw,72px)] pb-20">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1280px]">
          <span className="mb-4 flex items-center gap-3 text-sm font-bold tracking-[0.2em] text-[#C9A86A] uppercase">
      ...
      <div className="mx-auto max-w-[1280px] px-[clamp(20px,5vw,72px)] py-[clamp(80px,10vw,140px)]">
        ...
        <div className="mb-[clamp(60px,8vw,100px)] grid grid-cols-1 gap-10 lg:grid-cols-[1fr_1fr] lg:items-center lg:gap-20">
          <div>
            <span className="mb-4 flex items-center gap-3 text-sm font-bold tracking-[0.18em] text-[#C9A86A] uppercase">
              <span className="h-[2px] w-8 bg-[#C9A86A]" />
              What Drives Us
            </span>
            <h2 className="text-[clamp(2.5rem,4vw,4rem)] font-bold leading-[1.05] tracking-tight text-[#3F6F63]">
              Food with purpose.
            </h2>
          </div>
          ...
          <div className="relative pl-6 lg:pl-10">
            ...
            <div className="absolute bottom-0 left-0 top-0 w-[2px] bg-gradient-to-b from-[#C9A86A] to-transparent" />
            <p className="mb-4 text-[1.1rem] leading-[1.8] text-[#3F6F63] font-medium">
            ...
            <p className="text-[1.05rem] leading-[1.8] text-[#6D706B]">
        ...
        <div className="grid grid-cols-1 items-center gap-[clamp(60px,8vw,100px)] lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="relative w-full max-w-[600px] mx-auto lg:max-w-none">
            ...
            <div className="absolute -bottom-6 -left-6 top-6 right-6 rounded-[2rem] border-[1.5px] border-[#C9A86A]/40 transition-transform duration-700 hover:-translate-x-2 hover:-translate-y-2 lg:-bottom-8 lg:-left-8 lg:top-8 lg:right-8" />
            
            {/* Main Image Container */}
            <div className="group relative aspect-[4/5] w-full overflow-hidden rounded-[2rem] bg-[#244C43] shadow-2xl">
              <Image
            ...
            <div className="absolute -bottom-10 -right-4 z-20 w-[85%] max-w-[380px] rounded-[1.5rem] bg-[#3F6F63] p-8 text-white shadow-2xl shadow-[#3F6F63]/30 transition-transform duration-500 hover:-translate-y-2 sm:-right-10 lg:-right-16 lg:p-10">
              <div className="mb-4 h-[2px] w-12 bg-[#C9A86A]" />
              <p className="mb-2 text-xs font-bold tracking-[0.2em] text-[#C9A86A] uppercase">
                Built On
              </p>
          ...
          <div className="flex flex-col justify-center lg:pl-10 pt-10 lg:pt-0">
            <h3 className="mb-8 text-2xl font-bold text-[#3F6F63]">
              Core Operations
            </h3>
            <div className="flex flex-col border-t border-[#E5DED1]">
              {overviewHighlights.map((item, index) => (
                <article
                  className="group relative flex cursor-default flex-col justify-center border-b border-[#E5DED1] py-8 transition-colors duration-500 hover:border-[#C9A86A]"
                  key={item.title}
                >
                  {/* Sliding Left Gold Accent */}
                  <div className="absolute left-0 top-0 h-full w-0 bg-[#C9A86A]/10 transition-all duration-500 ease-out group-hover:w-full" />
                  <div className="absolute left-0 top-0 h-full w-[3px] scale-y-0 bg-[#C9A86A] transition-transform duration-500 ease-out group-hover:scale-y-100" />
                  ...
                  <div className="relative z-10 flex items-start gap-6 pl-4 transition-transform duration-500 group-hover:translate-x-2 lg:pl-6">
                    <span className="mt-1 font-mono text-sm font-bold text-[#C9A86A]">
                      0{index + 1}
                    </span>
                    <div>
                      <h4 className="mb-2 text-[1.1rem] font-black tracking-[0.1em] text-[#3F6F63] uppercase transition-colors duration-300">
                        {item.title}
                      </h4>
                      <p className="max-w-[90%] text-[1.05rem] leading-[1.6] text-[#6D706B] transition-colors duration-300 group-hover:text-[#3F6F63]">
                        {item.description}
                      </p>
                    </div>
                  </div>
