      ...
      <div className="mx-auto max-w-[1280px]">
        <div className="mb-[clamp(36px,6vw,64px)] grid grid-cols-1 gap-6 lg:grid-cols-[0.85fr_1fr] lg:items-end">
          <div>
            ...
            <h2 className="max-w-[620px] text-[clamp(2rem,4vw,3.6rem)] font-bold leading-[1.05] text-[#3F6F63]">
              Reach the MHPL team.
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          {offices.map((office) => (
            <article
              className="rounded-[1.5rem] border border-[#E5DED1] bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#C9A86A] hover:shadow-xl hover:shadow-[#3F6F63]/10"
              key={office.title}
            >
              <p className="text-[0.78rem] font-bold tracking-[0.16em] text-[#C9A86A] uppercase">
                {office.title}
              </p>
              <p className="mt-5 text-[1.15rem] font-bold leading-[1.55] text-[#3F6F63]">
                {office.address}
              </p>
            </article>
          ))}
